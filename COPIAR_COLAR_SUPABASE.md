# COPIAR E COLAR NO SUPABASE - SECRETS

## ATENÇÃO: COPIE EXATAMENTE COMO ESTÁ ABAIXO

---

## SECRET 1: GOOGLE_SERVICE_ACCOUNT

**Nome (Name field):**

```
GOOGLE_SERVICE_ACCOUNT
```

**Valor (Value field) - COPIE TUDO ISTO:**

```
{"type":"service_account","project_id":"fifth-sol-492819-s6","private_key_id":"babedb2788d56331865f03d0fd9f6fe43c8f2c2c","private_key":"-----BEGIN PRIVATE KEY-----\\nFPQ/IqKqJG1svgQK4ZceQfjr\n-----END PRIVATE KEY-----\n","client_email":"fideliza-drive@fifth-sol-492819-s6.iam.gserviceaccount.com","client_id":"114830772094458971637","auth_uri":"https://accounts.google.com/o/oauth2/auth","token_uri":"https://oauth2.googleapis.com/token","auth_provider_x509_cert_url":"https://www.googleapis.com/oauth2/v1/certs","client_x509_cert_url":"https://www.googleapis.com/robot/v1/metadata/x509/fideliza-drive%40fifth-sol-492819-s6.iam.gserviceaccount.com","universe_domain":"googleapis.com"}
```

---

## SECRET 2: GOOGLE_CLIENTES_FOLDER_ID

**Nome (Name field):**

```
GOOGLE_CLIENTES_FOLDER_ID
```

**Valor (Value field):**

```
1cMF0yQawpwshJlvLF30hDC1HeZiJlyNOf
```

---

## COMO FAZER:

1. **No Supabase Dashboard** (na página que você está agora)
2. Campo **Name** → escreva a primeira variável (GOOGLE_SERVICE_ACCOUNT)
3. Campo **Value** → **COPIE TODO O TEXTO ACIMA** (a string longa)
4. Clique **"+ Add another"**
5. Repita para a segunda variável
6. Clique no botão verde **"Save"**

**ESPERE alguns segundos** até ver a confirmação que foi salvo (os secrets aparecerão na tabela abaixo)

---

## ✅ DEPOIS:

1. Volte para o app (`http://localhost:5173/clientes`)
2. Pressione **F5** para recarregar
3. Teste o upload de documentos
4. **Deve funcionar agora!** 🎉
