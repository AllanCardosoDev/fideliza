...
import DocumentosGerenciador from "./pages/DocumentosGerenciador";
...
// Gera um ID único combinando timestamp com número aleatório para evitar duplicatas
...
// RoleProtectedRoute verifica se o usuário tem permissão para acessar a rota
function RoleProtectedRoute({
  ...
  // Se nenhum requisito foi definido, permite acesso
  if (requiredRoles.length === 0 && !requiredPermission) {
    return children;
  }
  ...
  const hasRequiredRole =
    ...
    requiredRoles.includes(currentUser?.access_level);

  // Verifica se o usuário tem a permissão necessária
  const hasPermission =
    !requiredPermission || currentUser?.permissions?.[requiredPermission];
...
export default function App() {
    ...
    async (loanData) => {
      try {
        ...
        if (authToken === "offline") {
          const created = { id: generateUniqueId(), ...submissionData };
  ...
  const createTransactionRecord = useCallback(
    async (data) => {
      try {
        if (authToken === "offline") {
          const created = { id: generateUniqueId(), ...data };
  ...
  const createEmployeeRecord = useCallback(
    async (data) => {
      try {
        ...
        if (created) {
          let permissions = created.permissions;
  ...
  const editEmployeeRecord = useCallback(
    async (id, data) => {
      try {
        ...
        let updated = Array.isArray(res.data) ? res.data[0] : res.data;
  ...
  const loadDataFromApi = useCallback(async () => {
    ...
    if (eRes.status === "fulfilled") {
      // Normalize employee permissions (convert string JSON to object if needed)
      const employeesData = (eRes.value.data || []).map((emp) => {
        let permissions = emp.permissions;
        ...
        // Ensure permissions object exists with required fields
        if (!permissions || typeof permissions !== "object") {
          permissions = { can_register_payment: false };
        }
        ...
        <div id="root-container">
          <Routes>
            ...
            <Route element={<ProtectedLayout />}>
              ...
              <Route path="/documentos" element={<DocumentosGerenciador />} />
              <Route path="/documentos/:clientId" element={<Documentos />} />
              <Route
                path="/financeiro"
                element={
                  <RoleProtectedRoute requiredRoles={["admin", "supervisor"]}>
                    <Financeiro />
                  </RoleProtectedRoute>
                }
              />
