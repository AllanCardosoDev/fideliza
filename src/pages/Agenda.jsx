...
function Agenda() {
  ...
  const allEvents = useMemo(() => {
    ...
    loans.forEach((loan) => {
      ...
      if (
        loan.status === "active" &&
        !loan.created_at?.includes(loan.updated_at)
      ) {
        const approvedDate = loan.updated_at
          ? loan.updated_at.split("T")[0]
          : new Date().toISOString().split("T")[0];
        const clientName = getClientName(loan.client);

        // Show to: creator (if employee), or admin/supervisor (if viewing others)
        const canSeeApproved =
          userRole === "admin" ||
          userRole === "supervisor" ||
      ...
      if (loan.status === "cancelled") {
        const rejectedDate = loan.updated_at
          ? loan.updated_at.split("T")[0]
          : new Date().toISOString().split("T")[0];
        const clientName = getClientName(loan.client);
  ...
  return (
    <div className="page active">
      ...
      {nextEvents.length > 0 && (
        <div
          className="card animate-in"
          style={{ marginBottom: 16, padding: "16px", "--delay": 4 }}
        >
